...
defmodule Edgehog.Campaigns.Executors.FirmwareUpgradeExecutorTest do
  ...
  describe "Executor immediately terminates" do
    test "when the firmware upgrade campaign has no targets", %{tenant: tenant} do
    ...
    test "when firmware upgrade campaign is already marked as failed", %{tenant: tenant} do
      campaign =
        1
    ...
    test "when firmware upgrade campaign is already marked as successful", %{tenant: tenant} do
      campaign =
        1
        |> campaign_with_targets_fixture(tenant: tenant, mechanism_type: :firmware_upgrade)
        |> Ash.load!(campaign_targets: [], campaign_mechanism: [])
  ...
  describe "Executor resumes :in_progress firmware upgrade campaign" do
    test "when it already has `max_in_progress_operations` pending updates", %{tenant: tenant} do
  ...
  describe "Executor sends" do
    test "all target OTA Requests in parallel if there are enough available slots", %{
      ...
      campaign =
        ...
        |> Ash.load!(
          campaign_targets: [device: [:device_id]],
          campaign_mechanism: [firmware_upgrade: [:base_image]]
        )

      parent = self()
      ref = make_ref()
      base_image_url = campaign.campaign_mechanism.value.base_image.url
    ...
    test "at most `max_in_progress_operations` OTA Requests", %{tenant: tenant} do
      target_count = Enum.random(10..20)
      max_in_progress_operations = Enum.random(2..5)
      ...
      # Expect max_in_progress_operations OTA Requests
      ref = expect_ota_requests_and_send_sync(max_in_progress_operations)
  ...
  describe "Executor receiving an OTAOperation update" do
  ...
  describe "Executor marks firmware upgrade campaign as successful" do
    setup %{tenant: tenant} do
      target_count = Enum.random(10..20)
      # 20 < x <= 70
      ...
      %{pid: pid, ref: ref} = start_and_monitor_executor!(campaign, start_execution: false)
    ...
    test "if all targets are successful", ctx do
      ...
      mark_all_pending_ota_operations_with_status(tenant, campaign_id, :success)
  ...
  describe "Executor marks firmware upgrade campaign as failed if `max_failure_percentage` is exceeded" do
    setup %{tenant: tenant} do
      target_count = Enum.random(10..20)
      # 20 < x <= 70
      max_failure_percentage = 20 + :rand.uniform() * 50
  ...
  describe "Executor terminates immediately" do
    setup %{tenant: tenant} do
      target_count = 1
    ...
    test "if there are no in progress targets when failure threshold is reached", ctx do
      ...
      [target] =
        MechanismCore.list_in_progress_targets(%FirmwareUpgrade{}, tenant.tenant_id, campaign_id)
  ...
  defp mark_all_pending_ota_operations_with_status(tenant, campaign_id, status) do
    %FirmwareUpgrade{}
    |> MechanismCore.list_in_progress_targets(tenant.tenant_id, campaign_id)
    |> Enum.each(fn target ->
  ...
  defp assert_campaign_outcome(tenant, id, outcome) do
    campaign = MechanismCore.get_campaign!(%FirmwareUpgrade{}, tenant.tenant_id, id)
