...
defmodule Edgehog.Campaigns.Executors.DeploymentStartExecutorTest do
  @moduledoc false
  ...
  alias Edgehog.Campaigns.CampaignMechanism.DeploymentStart
  alias Edgehog.Campaigns.CampaignMechanism.DeploymentStart.Executor
  ...
  describe "Executor immediately terminates" do
    test "when a deployment start campaign has no targets", %{tenant: tenant} do
      campaign = campaign_fixture(tenant: tenant, mechanism_type: :deployment_start)
    ...
    test "when deployment start campaign is already marked as failed", %{tenant: tenant} do
      campaign =
        1
        |> campaign_with_targets_fixture(tenant: tenant, mechanism_type: :deployment_start)
    ...
    test "when deployment start campaign is already marked as successful", %{tenant: tenant} do
      campaign =
        1
        |> campaign_with_targets_fixture(tenant: tenant, mechanism_type: :deployment_start)
        |> Ash.load!(campaign_targets: [], campaign_mechanism: [])
  ...
  describe "Executor resumes :in_progress deployment start campaign" do
    test "when it already has `max_in_progress_operations` pending start operations", %{
      tenant: tenant
    } do
      ...
      _ = expect_deployment_start_requests_and_send_sync(0)
  ...
  describe "Executor sends" do
    test "all target Start Deployment Requests in parallel if there are enough available slots",
         %{
           tenant: tenant
         } do
      target_count = Enum.random(2..20)
    ...
    test "at most `max_in_progress_operations` Start Deployment Requests", %{tenant: tenant} do
      target_count = Enum.random(10..20)
      max_in_progress_operations = Enum.random(2..5)
      ...
      ref = expect_deployment_start_requests_and_send_sync(max_in_progress_operations)
    ...
    test "Start Deployment Requests only to online targets", %{tenant: tenant} do
      target_count = Enum.random(10..20)
      ...
      ref = expect_deployment_start_requests_and_send_sync(online_count)
  ...
  describe "Executor receiving a Deployment update" do
    ...
    test "frees up slot if Deployment state is :started", ctx do
      ...
      ref = expect_deployment_start_requests_and_send_sync()
    ...
    for status <- [:pending, :sent] do
      test "doesn't free up slots if Deployment status is #{status}", ctx do
        ...
        expect(DeploymentCommandMock, :send_deployment_command, 0, fn _client,
                                                                      _device_id,
                                                                      _data ->
          :ok
        end)
  ...
  describe "Executor marks deployment start campaign as successful" do
    setup %{tenant: tenant} do
      target_count = 5
      # 20 < x <= 70
    ...
    test "if all targets already have the deployment started", ctx do
      %{
        release: release,
        executor_pid: pid,
        monitor_ref: ref,
      ...
      |> Enum.map(fn target -> fake_start(target.device, release, tenant) end)
  ...
  describe "Executor marks campaign as failed if `max_failure_percentage` is exceeded" do
    ...
    test "by failed Start Operations on Deployments", ctx do
      %{
        executor_pid: pid,
        failing_target_count: failing_target_count,
        monitor_ref: ref,
  ...
  describe "pause and resume deployment start executor" do
    test "pause suppresses new start requests and resume restarts rollout", %{tenant: tenant} do
      max_updates = 3
      ...
      # Expect initial max_updates start requests
      init_ref = expect_deployment_start_requests_and_send_sync(max_updates)
      ...
      # While paused, no further start requests should be sent
      ...
      # Now resume - expect new start requests for remaining targets
      # (8 total - 3 completed = 5 remaining, capped at max_updates = 3)
      resume_ref = expect_deployment_start_requests_and_send_sync(max_updates)
      ...
      executor_id = {tenant_id, campaign_id, :deployment_start}
    ...
    test "campaign can complete while paused", %{tenant: tenant} do
      ...
      # Mark all pending as running; executor will transition to campaign_paused, then campaign_success
      %DeploymentStart{}
  ...
  defp fake_start(device, release, tenant) do
    {:ok, deployment} = fetch_deployment(device, release)
    update_deployment_state!(tenant, deployment.id, :started)
  end
  ...
  defp expect_deployment_start_requests_and_send_sync(count \\ 1) do
  ...
  defp assert_campaign_outcome(tenant, id, outcome) do
    campaign = MechanismCore.get_campaign!(%DeploymentStart{}, tenant.tenant_id, id)
