...
defmodule Edgehog.Campaigns.Executors.DeploymentDeployExecutorTest do
  @moduledoc false
  ...
  describe "Executor immediately terminates" do
    test "when a deployment deploy campaign has no targets", %{tenant: tenant} do
    ...
    test "when deployment deploy campaign is already marked as failed", %{tenant: tenant} do
      campaign =
        1
    ...
    test "when deployment deploy campaign is already marked as successful", %{tenant: tenant} do
      campaign =
        1
        |> campaign_with_targets_fixture(tenant: tenant, mechanism_type: :deployment_deploy)
        |> Ash.load!(campaign_targets: [], campaign_mechanism: [])
  ...
  describe "Executor resumes :in_progress campaign" do
    test "when it already has `max_in_progress_operations` pending deployments", %{tenant: tenant} do
  ...
  describe "Executor sends" do
    ...
    test "at most `max_in_progress_operations` Deployment Requests", %{tenant: tenant} do
      target_count = Enum.random(10..20)
      max_in_progress_operations = Enum.random(2..5)
      ...
      ref = expect_deployment_requests_and_send_sync(max_in_progress_operations)
  ...
  describe "Executor marks deployment deploy campaign as successful" do
    setup %{tenant: tenant} do
      target_count = Enum.random(10..20)
      # 20 < x <= 70
  ...
  describe "pause and resume deployment deploy executor" do
    test "pause suppresses new deployment requests and resume restarts rollout", %{tenant: tenant} do
      max_updates = 3
      ...
      # Expect initial max_updates deployment requests
      init_ref = expect_deployment_requests_and_send_sync(max_updates)
      ...
      # Now resume - expect new deployment requests for remaining targets
      # (8 total - 3 completed = 5 remaining, capped at max_updates = 3)
      resume_ref = expect_deployment_requests_and_send_sync(max_updates)
      ...
      executor_id = {tenant_id, campaign_id, :deployment_deploy}
  ...
  defp assert_campaign_outcome(tenant, id, outcome) do
    campaign = MechanismCore.get_campaign!(%DeploymentDeploy{}, tenant.tenant_id, id)
