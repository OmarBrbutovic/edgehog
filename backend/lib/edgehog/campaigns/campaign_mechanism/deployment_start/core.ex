...
defimpl Edgehog.Campaigns.CampaignMechanism.Core,
  for: Edgehog.Campaigns.CampaignMechanism.DeploymentStart do
  @moduledoc """
  Core implementation for Start Operation on deployment campaign execution.
  ...
  def do_operation(mechanism, target) do
    start(target, mechanism.release)
  end

  defp start(target, release) do
  ...
  @doc """
  Retries the start operation for a target.
  ...
  ## Returns
    - The fully loaded deployment start mechanism with release data.
  """
  def get_mechanism(_mechanism, campaign) do
    mechanism =
      ...
      |> Ash.load!(
        campaign_mechanism: [
          deployment_start: [release: [containers: [:networks, :volumes, :image]]]
        ]
      )
