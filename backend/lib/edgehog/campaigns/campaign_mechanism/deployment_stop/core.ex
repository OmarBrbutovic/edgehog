...
defimpl Edgehog.Campaigns.CampaignMechanism.Core,
  for: Edgehog.Campaigns.CampaignMechanism.DeploymentStop do
  @moduledoc """
  Core implementation for Stop Operation on deployment campaign execution.
  ...
  def do_operation(mechanism, target) do
    stop(target, mechanism.release)
  end

  defp stop(target, release) do
  ...
  defp maybe_stop(deployment, updated_target) do
    deployment_result =
      deployment
  ...
  @doc """
  Retries the stop operation for a target.
  ...
  def retry_operation(_mechanism, target) do
    Helpers.do_retry_target_operation(target, :stop)
  end
  ...
  ## Returns
    - The fully loaded deployment stop mechanism with release data.
  """
  def get_mechanism(_mechanism, campaign) do
    mechanism =
      ...
      |> Ash.load!(
        campaign_mechanism: [
          deployment_stop: [release: [containers: [:networks, :volumes, :image]]]
        ]
      )
