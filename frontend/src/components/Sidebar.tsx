...
const SidebarItem = memo(
  ({
    ...
    return (
      <NavLink
        as={Link}
        className={`sidebar-link d-flex align-items-center mx-2 px-3 py-2 rounded ${
...
const SidebarItemGroup = memo(
  ({ children, icon, label, eventKey, isCollapsed }: SidebarItemGroupProps) => {
    const location = useLocation();
    ...
    const handleMenuClick = useCallback(() => setIsOpen(false), []);
      ...
      <Accordion defaultActiveKey={eventKey} alwaysOpen className="w-100">
        <Accordion.Item eventKey={eventKey} className="border-0">
          <Accordion.Header className="m-2">
            <div className="d-flex align-items-center w-100 py-2 rounded text-dark">
        ...
        {!isMobileMenuOpen && (
          <div className="sidebar-header d-flex align-items-center justify-content-between px-4 shadow-sm">
            <Image
              ...
              src={
                isDesktopCollapsed ? assets.images.logo : assets.images.brand
              }
